---
description: DAW-Gate-Definition - Define verification gates for an issue (MANDATORY before task planning)
---
...
This is an ESTIMATE only - actual task planning happens in `/02-task-plan`
...
- Review gates.md
- If approved: `/02-task-plan <issue-identifier>`

If splitting recommended and accepted:
- Return to `/01-issue-plan` to create sub-issues
- Then run `/define-gates` for each sub-issue
